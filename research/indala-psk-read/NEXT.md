...
|---|---|
| ⛔ **OPEN, and it needs an instrument this bench does not have: the reader path sees 1.8x the signal the sniff path does** | ⭐ Both go through the same `capture_begin`, the same `init_saadc_hw` and the same phase and gain setters, yet on the same tag in the same minute the reader measures a mean boundary step of ~5070 and the sniff measures 2819 — and the sniff capture decodes exactly on the host (C209). Phase, settle, gain, sample width, splicing and timing are all ruled out by measurement. ⇒ What would settle it is seeing the READER's own samples, which needs a command that runs the reader's capture and returns the buffer rather than decoding it. That is a firmware change plus a host tool, and it is the next thing to build if GProxII's read arm is wanted |
