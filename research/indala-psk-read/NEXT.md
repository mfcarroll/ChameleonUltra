...
2. **C400 — HALF CLOSED.** ✅ **Gallagher does NOT reproduce (C445)**: pm3-written tag, judged twice, **#2 reads 5/5 byte-exact** (Region 1 / Facility 4321 / Card 6789 / Issue 2) against C400's 0/6 — and `git log 885b2f24..HEAD -- firmware/application/src/rfid/reader/lf/` is **EMPTY**, so the code is unchanged and it was **confounded, not fixed**. ✅ The engine is up: Indala reads 5/5 on a real tag (C444). ⚠ **SECURAKEY IS THE ONLY PART LEFT OPEN**, and it needs one thing first: `lf securakey clone` takes **only a 12-byte raw** and the notes hold none. ⭐ Get it the honest way — write a Securakey tag from our own `lf securakey write`, let **pm3 demod it and report its raw**, then use that pm3-supplied raw to re-clone with pm3 so the tag is independently written for the actual test. ⛔ Do not invent a raw and do not read our own value back as if it were a judgement. ⚠ Run Gallagher or Indala alongside as the engine-up control — a Securakey null with no passing SAADC arm in the same session is exactly how C400 became unattributable.
