...
✅ 0. **ANSWERED 2026-09-15 BY C472 — F12/U11 IS REFUTED, AND THE SILENCE WAS THE FLIPPER'S.**
   The Proxmark decodes **all three** FSK2a emulate arms BYTE-EXACT, on **both** devices and
   **both** firmware builds — 6 of 6, five different credentials, every raw identical to what was
   armed. A constant tone cannot carry a credential. ⛔ Every empirical leg of F12 was a FLIPPER
   capture (C245/C246, C387, C409, C413, C420, C422/C423); the Flipper reads a REAL FSK2a tag fine
   (C413: 44.4%) but cannot tolerate our emission's shape, exactly as C465 found for real PAC.
   ⭐ **The shape difference is real and is now measured properly**: 48us runs at 16.4% where the
   real tag has 0.01%, duty 52.9/47.1 against the tag's 50.0/50.0 — `LF_FSK2A_MARK_CYCLES = 4`,
   one 32us mark on both tones. C414/C420's mechanism is confirmed as a description of the
   WAVEFORM and refuted as an explanation of UNDECODABILITY.
...
and also matches, both under a field verified by `playbacks` rising. **So every step from the
...
1. **Diagnose the PAC emulate arm.** It was never in C378's 11-protocol survey, so this is an unmeasured arm,
   ...
   ✅✅ **THE FRAME IS PROVEN AND THE WRITE AND READ ARMS ARE GRADE A (C436).** `pac_t55xx_writer`'s
   blocks were predicted from source before the bench — **00080080 / FF204990 / 6D8541C9 / 511C1B06 /
   C1B46551** — written by #2 over a tag that held HID H10301, and **pm3's dump matches all five
   byte-exact**. pm3's own PAC/Stanley decoder returns `CARD0001` on two acquisitions, and our read arm is
   **5/5 byte-exact** on that tag. ⇒ `pac_build_bitstream` is correct, so **the emulate suspects are now
   the MODULATOR or the AIR PATH only**.
   ⛔⛔ **DO NOT ACT ON C435's INSTRUMENT CLAIM — IT IS RETRACTED (C436).** The Flipper measures
   levels fine; the 0.3-2.6% figure compared uncorrected runs against corrected periods. Bias-corrected, runs
   quantise at 83-89% within 0.15 bit. **M54 was rewritten.** ⚠ But the run bias is genuinely **not a
   constant** (93us and 151us minutes apart) — fit it per capture.
   ...
   **16 / 11 / 17**, each `ceil(500000 / (entries × counter_top × 8))` from that arm's own header.
   ...
   ✅✅ **AND THE CLOCK IS RIGHT TOO (C455) — do not re-fit it.** `quantum.py` finds the emitted
   PERIODS are integer multiples of **256us**, PAC's own bit period, at residual **0.033** and
   **0.018** for two credentials — **tighter than the gproxii control at the same quantum** (0.056),
   with fdxb landing on its predicted 128us and a uniform-random null scoring 0.234-0.239 in every
   capture. ⚠ Any divisor fits as well and a loose band passes at small n: **the minimum is the
   reading, not the band**.
...
2. ✅✅ **C400 IS CLOSED AND RETRACTED IN FULL (C445, C446) — do not re-open it as an open unit.** Gallagher **5/5** and Securakey **5/5** on pm3-written tags, each judged twice by two different pm3 arms, with Indala **5/5** (C444) proving the SAADC engine up. The LF reader path is **byte-identical** to what C400 tested — `git log 885b2f24..HEAD -- firmware/application/src/rfid/reader/lf/` is empty — so nothing was fixed and the original measurement was wrong. ⚠ The confound is **unidentified and not asserted**; C400 shares a date with the bench-topology confusion behind C402/C404/C408. ⭐ The lesson worth carrying is M28's: Securakey's verdict had to be **value-gated**, because our arm prints a raw Wiegand line rather than a branded one and a name test would have scored a passing arm 0/5 — C177's exact defect.
