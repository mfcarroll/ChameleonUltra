...
|---|---|
| ⛔⛔ **CHAMELEON #2 AND THE FLIPPER HAVE DROPPED OFF USB — and that is a lead on the coupling intermittent** | ⭐ 2026-09-12: only rig A's Chameleon and the Proxmark enumerate. A rig-B Chameleon that comes and goes would explain a tag that stops answering while our own field still measures healthy (C163, C159, C155). ⚠ **A lead, not a diagnosis** — C163 measured that unit's transmit AND receive working minutes before it vanished, so this does not yet explain the two different signatures. ⇒ First thing to check: are they enumerated? Then take the fc/2 pair from C163 before blaming any code |
...
| ◐ **B. New protocols, by modulation family** — §10 | ✅ **Keri complete** — read, write, emulate, all on hardware. ◐ **NexWatch is next and is reconnoitred, not started**: Momentum's `protocol_nexwatch.c` gives 255 us/bit and 16 pulses/bit — the SAME RF/32, fc/2 air layer as the rest of this family — but a **96-bit** encoded frame, not 64, and an **8-bit preamble**. ⛔ Eight bits is far weaker than Indala's 33 or Keri's 33, and every false-credential failure this project has had (C90, C104, C157) came from a weak preamble. ⇒ Budget for `require_repeat` and cross-protocol vetoes from the start, and treat a preamble-only match as unacceptable | ⛔ **hands** — see the row above |
