...
|---|---|
| ⛔⛔ **THE TAG STOPS ANSWERING THE CHAMELEON, INTERMITTENTLY, ON A TIMESCALE OF MINUTES** | ⭐ Read C163 then C159. It read 6/6 and then, thirty minutes later with nobody touching the bench, `LF tag not found` — while the Proxmark read the same tag perfectly throughout. **Our field is provably healthy**: rms scales 298/417/596 across drives 1/4/7 and `hw lfdebug` is clean, but fc/2 stays at 0.1-0.4 at every drive. ⚠ **Two episodes, two different signatures** — C159 had signal down and noise UP, C163 has both at the floor — so the interference story is not settled. ⇒ **The diagnostic to run first, before blaming any code**: one `lf sniff` and the fc/2 amplitude. Below ~1 means nothing is answering, and no amount of firmware will change it |
