...
| **U16** | ⛔⛔ **WHY FDX-B NO LONGER READS.** 0 of 4 on the current build where C214 had 6/6. ⭐ **It is NOT the corroboration width** — the A/B was run: reverting only that comparison to 8 bytes leaves it at 0 of 4 (C334). ⇒ The break is one of the changes between C214 and now — the password fix, F5's capture-buffer merge, F6 — and none is ruled out. ⭐ `git log` the reader path between those points and bisect; the host path (`rdrcap.py` + `bidemod.py`) decodes captures without a reflash, and six of them already show a STABLE word, so the tag and the capture are probably fine and the fault is later | device (sandwich) | the breaking change identified and either reverted or the arm honestly re-graded |
