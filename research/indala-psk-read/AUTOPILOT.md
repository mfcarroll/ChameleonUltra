...
| **U18** | ⛔⛔⛔ **WHY THE EMULATOR'S PWM PLAYS SOMEBODY ELSE'S BUFFER (C367).** `hw lfdebug` reports `SEQ[0].PTR ours: False — PWM0 IS NOT READING OUR SEQUENCE` on every failing arm, with `ENABLE: 1` and the correct `125kHz` clock — twice stale, once null. ⛔ **NOT ASK-specific and NOT the clock**: EM410X (different emitter) fails too, and the clock is right. ⭐ EM410X emulated 3/3 earlier in the same session and 0/4 later on the same slot, so it is a STATE the device enters. ⇒ **Next: find what loads `m_pwm_seq` and what leaves it null** — the signature matches C129, which F10 claims to have fixed, and F10 is one of the two entries `fixcheck.sh` cannot verify without rig A. ⚠ Do not call it a regression until older firmware has been tested | device (rig A) | the loading path identified, and F10 either confirmed or reopened |
