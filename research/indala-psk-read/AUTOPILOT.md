...
| **U16** | ⛔⛔ **WHY FDX-B's TAIL DOES NOT REPEAT.** The corroboration fix (F7, C332) took FDX-B's read arm to 0 of 4 while the other six long-frame arms were unaffected, so its decoded back half differs between consecutive captures. ⭐ The instrument already exists: `rdrcap.py` pulls raw captures and the host decoder in `ctest` decodes them, so the two disagreeing words can be printed and diffed WITHOUT a reflash. ⛔ Do not 'fix' it by narrowing the comparison back — that is the defect, not the cure | device (sandwich) | the disagreeing bytes identified and either the decoder fixed and FDX-B reading 4 of 4, or the instability characterised and the arm honestly graded |
