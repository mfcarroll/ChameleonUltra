...
| **U16** | ⛔⛔ **WHY FDX-B NO LONGER READS — HALF ANSWERED (C335).** ⭐ **Measured:** it decodes **16 of 16** from a held field and **5 of 16** from a cycled one (p = 6.8e-5); failures are SATURATED, edge ~9000 against ~85. The reader cycles the field per capture — `capture_begin` starts the radio with a 2 ms settle every time. ⛔ **Not sufficient:** 31% per capture over 8 tries predicts 0.77 of two agreeing at one phase, so the device should usually read and it reads 0 of 4. Something else is also wrong. ⇒ **Next:** instrument the device scan itself (how many captures it actually takes inside the 3000 ms budget, and whether it reaches more than one phase) before changing the shared capture path — holding the field is a change every protocol pays for. ⚠ Judge with `cdemod --fdxb`; `rdrcap.py` defaults to `--drive 7` which is the WRONG drive for FDX-B | device (sandwich) | the remaining gap identified, then a fix that does not regress the other six long-frame arms |
