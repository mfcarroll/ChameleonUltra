...
| **U18** | ⛔⛔⛔ **BISECT THE ASK/MANCHESTER EMULATE REGRESSION (C366).** Gallagher, Securakey, Noralsy and HID Prox emit **nothing** on current firmware — 0 of 6 each — while PSK1 is 6/6 and EM410X 3/3 on the same slot in the same session, and the device reports the slot correctly configured and in emulator mode. The grid records these at 10/10 on older firmware (C167, C178). ⇒ **Not bisected.** `./emugrade.sh` reproduces it in one command; #1 is on current firmware and faces the Flipper, so this needs no bench change. ⚠ EM410X does not clear the path — it uses the GPIO/comparator emitter, not the ASK/Manchester one the failing four share | device (rig A) | the commit that broke it identified, or the arms honestly re-graded |
