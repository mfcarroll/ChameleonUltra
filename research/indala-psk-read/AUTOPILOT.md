...
| **U17** | ⭐ **HALF DONE — FDX-A READ IS NOW GRADE A (C338), WRITE IS SPECIFIED AND UNBUILT (C339).** Read: 10 of 10 on a real Proxmark-written tag across two credentials. ⇒ **Remaining: the writer.** Everything it needs is measured: config **`00105060`** = `T5577_BITRATE_RF_50 | T5577_MODULATION_FSK2 | (3 << T5577_MAXBLOCK_SHIFT)` (the constant already exists, unused), three data blocks, and the frame stored as the **bitwise complement** of what `lf fdxa read` prints. ⛔ **FSK2, NOT FSK2a** — one bit from AWID's `00107060`, and C171 says the wrong one writes a tag nothing here can read. Share `fsk2a_t55xx_blocks()` unchanged; add a `ctest/roundtrip.c` arm pinning the block vectors, as C203 did for the other three. ⇒ Verify with `lf destron reader` on a WIPED tag, changed credential | device (sandwich) | an FDX-A writer read back 4 of 4 by `lf destron reader`, on a wiped tag, with a credential different from the read tests |
