...
| **U16** | ⛔⛔ **WHY FDX-B NO LONGER READS — still open, and two of my own mechanisms have been withdrawn.** ⭐ **Stands:** it decodes **16 of 16** from a held field and **5 of 16** from a cycled one (p = 6.8e-5), and the reader cycles the field per capture (`capture_begin`, 2 ms settle). ⛔ **Withdrawn:** the saturation story (C336 — a truncated column) and the corroboration-width story (C334 — refuted by its own A/B). ⛔ **Ruled out:** SAADC phase — with the field held it decodes at all eight `PHASE_ROTATION` entries, 15 of 16. ⛔ **Still does not add up:** 31% per capture over 8 tries predicts 0.77 of two agreeing at one phase; the device reads 0 of 4. ⇒ **Next: instrument the device, do not theorise.** Report captures taken, phases reached and decodes seen on FAILURE — the numbers the host cannot see. A small firmware change and one flash. ⚠ Judge with `cdemod --fdxb` and print WHOLE lines; `rdrcap.py` defaults to `--drive 7`, the wrong drive here | device (sandwich) | the device's own scan counters in hand, then a cause that survives its own A/B |
| **U17** | ⭐ **REOPEN THE FDX-A WRITER.** C185 refused it as unverifiable because *pm3 has no FDX-A*; it has `lf destron` with demod, reader, **clone** and sim (C333). ⇒ pm3 can write a Destron tag for our reader AND read back one we write, which is exactly the independent check whose absence was the refusal | device (sandwich) | either an FDX-A writer verified 4 of 4 by `lf destron reader`, or a REFUSAL restated on a premise that survives checking |
