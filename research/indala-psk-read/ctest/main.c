...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (modefa) {
            lf_decode_result_t rf;
            if (!fdxa_fsk_decode(buf, n, &rf)) {
                printf(" %-44s %5zu samples  -                          bits %4ld\n",
                       argv[i], n, (long)rf.energy);
                continue;
            }
            decoded++;
            uint8_t pay[5];
            fdxa_fsk_payload(rf.word_bits, pay);
            printf(" %-44s %5zu samples  ", argv[i], n);
            for (int k = 0; k < 12; k++) printf("%02x", rf.id[k]);
            printf("  payload ");
            for (int k = 0; k < 5; k++) printf("%02x", pay[k]);
            printf("\n");
            continue;
        }
        ...
        if (modeif) {
            ...
            decoded++;
            printf(" %-44s %5zu samples  ", argv[i], n);
            for (int k = 0; k < 29; k++) printf("%02x", ri2.id[k]);
            printf("  ph %2u pos %3u %s\n", ri2.offset, ri2.bit_pos, ri2.inverted ? "inv" : "");
            continue;
        }
        ...
        if (modesk) {
            ...
            decoded++;
            printf(" %-44s %5zu samples  ", argv[i], n);
            for (int k = 0; k < 12; k++) printf("%02x", rs.id[k]);
            printf("  ph %2u pos %3u %s\n", rs.offset, rs.bit_pos, rs.inverted ? "inv" : "");
            continue;
        }
