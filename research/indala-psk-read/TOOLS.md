...
| `pm3grade.sh` | ⭐⭐⭐ Grades the whole **emulate** column with the **PROXMARK** as reader — the run that produced C473's 16-of-16. One Chameleon alone on the pm3's pad, 16 arms scored BYTE-EXACT against what was armed, null sweep of every decoder before AND after. ⛔ Set `EMU` to the device you actually put there: the null arms catch a stray emitter, NOT a swapped one (C461/C472). ⚠ Its silences are PROBE results — the pm3 and the Flipper have complementary decoder gaps, so a SILENT arm here is only an emitter claim if the other reader is silent too, and for no arm is that currently true |
| `emugrade.sh` | ⚠ Flipper-as-reader version. ⛔ Its header claims *the Proxmark cannot hear our emulation at all* — FALSE, refuted by C466 and comprehensively by C472/C473; use `pm3grade.sh` as the second judge. ⭐ Grades the whole **emulate** column on rig A, Flipper as reader. Aborts if the reader is not proven alive first (C373/C374 — a silent reader and a silent emulter give identical numbers) |
...
| ⛔⛔ **`seqdump.py` — IT HAS NEVER WORKED: IT ARMS 1 OF 5 STEPS (2026-09-16).** Every `VOID — no reader field reached the device` it has printed was this, NOT the field — a pm3 `lf tune -n 90` sustaining **21 V** produced a void dump anyway. A slot emits only when all five are done in order: `hw slot type -s N -t TYPE`, `hw slot enable -s N --lf`, the econfig, `hw slot change -s N`, `hw mode -e`, then ~1s settle (the sequence is in rfid-tools `benchmatrix/devices.py:589`). `seqdump.py` does the econfig alone. ⇒ FIX THIS BEFORE TRUSTING ANY DUMP. ⛔ Disarm with `hw mode -r` in a `finally`: a Chameleon left emulating jams the Proxmark's entire pad, including its reads of a tag (L442). ⭐ **`seqdump.py` — read the PWM wave-form entries off the device and grade them against the emitter SOURCE** | The one step of the PAC path no air-side measurement can reach: four air routes are refuted, each by its own control (C440/C441/C451/C456), because the air carries what the PERIPHERAL made of the buffer, never the buffer. ⛔ Needs firmware carrying `DATA_CMD_LF_EMU_SEQDUMP` (3065) — **not yet flashable, see C459** | `./seqdump.py pac gprox` — gproxii goes through the SAME command (96 entries at counter_top 64 vs pac's 128 at 32) so a constant or stale answer cannot pass. ⛔ Guards on `playbacks` RISING, not the `emulating` flag (C460) |
