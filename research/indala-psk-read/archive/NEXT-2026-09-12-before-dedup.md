<!-- FROZEN 2026-09-12. Snapshot of NEXT.md immediately before the de-duplication pass.

     995 lines, of which roughly 600 are DUPLICATED sections — an editing bug, not
     accumulated history. Repeated `s[:a] + new + s[b:]` splices matched the FIRST occurrence
     of an anchor while copies already existed, so each edit appended another copy instead of
     replacing. §3b appears six times, §3a five. checkdocs.sh validated every cross-reference
     in it and never noticed, because it checked references rather than duplication.

     Kept so the pre-cleanup state is recoverable and the failure mode stays visible.
     Do not edit. -->
...
**Where it stands:** ⭐ **The burst boundary was the cause** (C75). `lf_tag_em.c` played 10
frames = 163.84 ms then paused to check the field; the Proxmark decoded any window inside one
burst and failed across a boundary. At 32 frames (524 ms) the cliff moved with it — 197 ms
and 262 ms both went ✗ -> ✓. ⭐ And the **USB cable costs ~40%**: unplugged the emulator
reaches 1.00x a real tag (C72 retracted).

**Next, in order:**

1. ⚠ **Pick the burst length deliberately — 32 was a guess that happened to work.** It buys
   a longer window but does not remove boundaries; a capture spanning one still fails, which
   is why 290 ms still does. The trade is **field-loss latency**: the field is only
   re-checked between bursts, so 32 frames means the device keeps modulating for ~524 ms
   after the reader leaves. Measure what readers actually need before settling it. ⛔ Do NOT
   simply maximise it — battery and responsiveness pay, and `NRFX_PWM_FLAG_LOOP` (no
   boundaries at all) was already tried and broke field detection via self-drive on LF_RSSI.
2. **Test against the other two readers.** The Chameleon's own `lf indala read` on the second
   unit, and the Flipper. Three independent decoders, two sharing nothing with our code, is
   the strongest bar available without a wall reader.
3. **Then re-test IDTECK**, whose retraction was collateral — nobody has shown it working or
   broken on a trustworthy instrument, and it shares this transmit path.

⭐ **Carrier locking (C74) is no longer the next step, but it is still the RIGHT fix.** It
removes boundaries entirely rather than making them rarer, and it is what the Flipper does.
Keep it as the destination; the burst length is a mitigation.

## 3a-tooling. ⚠ Drive the Chameleon over BLE instead of USB

The cable costs ~40% of the emulated signal (C72), which makes every cabled emulation
measurement pessimistic and forces a physical unplug between configuring the device and
testing it — the exact gap that produced three empty-field results in one session.

The firmware already speaks BLE (the mobile app uses it; `g_is_ble_connected` gates the
advertising suspend in `lf_reader_generic.c`). **The Python CLI does not** — `chameleon_com.py`
is serial-only, with no `bleak` dependency anywhere in `software/script`. So this is a new
transport behind the existing command layer, not a flag.

⚠ Worth knowing before starting: a live BLE connection during emulation is a condition nobody
has characterised, and C47 records that advertising bursts collapse the 125 kHz field. Driving
over BLE might introduce its own artefact — measure the emulated signal with and without a BLE
connection before trusting it as the test path.
