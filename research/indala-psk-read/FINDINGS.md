...
| C26 | ⛔ **A second, Proxmark-verified Indala tag was INAUDIBLE**: 0/12 decodes, and its fc/2 skirt sits at **0.89–1.11x the empty-field floor across 12 sample phases** where the bench tag sits at 1.25–1.73x. Nothing arrives; this is not a lock failure | 12 phases | ✓ empty-field captures at the *same* phases | ✓ the Proxmark wrote, verified and read the tag back | L53 |
| C27 | ⇒ **The working margin is tiny: 1.25–1.73x over the floor is the whole operating range.** Anything costing more than ~1.3x makes a tag unreadable | 12 | ✓ per-phase empty reference | — | L53 |
...
| Does a second **physical** tag read? | ⛔ **NO, on the one tried** — C26. Cause unknown: position, tag, or reader, in that order of suspicion. The control that separates them (put the bench tag back and confirm it still reads) has NOT been run, so "this tag is inaudible" and "nothing reads any more" are currently the same observation |
