...
| C63 | ⭐⭐ **Indala EMULATION transmits, at the right frame rate, with the phase information destroyed — and the modulator is provably not at fault.** Sniffed from a second Chameleon in reader mode. The emulator is modulating: fc/2 band **62524 against an empty field's 6399** (9.8x), and the frame autocorrelates at **lag 2070** where a 64-bit RF/32 frame is 2048. But the longest constant-phase run is **102 samples (3.2 bits)** where `a0000000e6bd0e92`'s 28-zero preamble demands **896**; a real tag in the same measure shows **912**. ⇒ The envelope timing survives and the DATA does not | 3 captures + 2 committed references | ✓ empty-field capture in the same session, same reader | ✓ the modulator was tested in isolation on the host: polarity-per-bit matches the frame EXACTLY, 16/16 entries per bit agree, counter_top 16, duty 8 | L71 |
| C64 | ⭐ **Emulation is front-only too.** fc/2 skirt 49236 with the emulator against the reader's front, **5049 on the other side** — against an empty floor of 4591. So the other side is indistinguishable from no tag at all. Same asymmetry as reading (C36), now shown for transmit | 2 orientations + empty | ✓ empty control in the same session | ⚠ n=1 pair, and which device's orientation dominates was not separated | L71 |
...
| C66 | ⛔⛔⛔ **PSK1 TAG EMULATION HAS NEVER WORKED ON THIS DEVICE — IDTECK IS BROKEN THE SAME WAY, AND IT PREDATES THIS PROJECT ENTIRELY.** Paired test, same frame `4944544b00000000`, same reader, same session: a **real** IDTECK tag gives a **1042-sample** constant-phase run against the 1024 the frame demands, autocorrelating at lag **2048**. The **emulated** one gives **100 samples** at lag 2071 — numerically indistinguishable from the Indala emulator's 102 at lag 2070 (C63). Both modulate (skirt 70183 and 62524 against an empty 4591) and neither carries phase. ⇒ It is not `indala.c`: the Indala emulator inherited a transmit path that was already broken | 3 captures, paired real-vs-emulated on one frame | ✓ empty control, and the REAL tag is the control on the instrument — 1042 vs 1024 expected validates the measurement itself | ✓ the frame was chosen by computing its run length first; the tag already on the bench would have been a useless probe at 128 samples | L72 |
| C67 | ⚠ **The emulated subcarrier is offset from fs/2 by ~640 Hz, and that is now a real measurement rather than the discarded one.** Mixed-baseband peak: **641 Hz** (IDTECK emulator), **671 Hz** (Indala emulator), against noise-dominated junk at 18372 Hz for an empty field. A ~640 Hz beat flips the apparent phase every ~780 us ≈ 98 samples, which is exactly the 100–102-sample run both emulators show. ⛔ Not to be confused with C65, which discarded the *spectrum-peak-near-62500* estimator; this is the mixed baseband, where the empty field gives no comparable peak | 2 emulator captures + empty | ✓ empty field produces no such peak | ⚠ 640 Hz is 1.03% of 62.5 kHz — FAR too large for crystal tolerance (~20 ppm gives ~2.5 Hz), so the mechanism is NOT explained by two independent clocks | L72 |
