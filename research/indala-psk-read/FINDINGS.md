...
| Why does stacking not rescue the weak tag? | ⛔ open, and it is the most useful question left (C30). Stacking is worth 1.74x on the bench tag and 1.0x on the white coin. C31 says the alignment test everything rests on is measuring the turn-on transient, so the real cross-capture alignment of either tag is currently unknown |
