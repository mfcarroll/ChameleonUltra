...
| C45 | ⛔ **`lf hid prox read` FAILS ON A 90x SIGNAL — so the intermittent HID read is the decoder, not the RF.** 0 of 6 reads while the tag measured 6.3M against a 63k floor. This is the exact discrimination `lfprobe.py` was built to make (amplitude flat while reads fail -> firmware) and it is now unambiguous: not coupling, not detuning, not placement, not thermal | 6 reads bracketing 10 Indala reads, with continuous amplitude | ✓ amplitude measured continuously across the failures | ⚠ not yet confirmed against a Proxmark read of the same tag, which would separate a malformed tag from a broken decoder | L59 |
