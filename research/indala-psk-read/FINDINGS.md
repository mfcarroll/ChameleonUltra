...
| C47 | ⚠ **HYPOTHESIS, NOT YET MEASURED: only `lf_reader_generic.c` suspends BLE advertising — every other LF reader captures with it running.** `advertising_stop()` appears in exactly 1 of 15 LF reader files. That file's own comment records why it was added: an advertising burst collapses the 125kHz field for ~1.6ms and hit **4 captures in 10**. `hidprox_read()` has no such guard. ⇒ It predicts the observation that has been unexplained since L51 — *"after connecting it to my phone and/or a reboot, it does read"* — because **a BLE central connecting is what stops advertising** | static read of 15 files; the field-collapse measurement is L-series, pre-existing | ⚠ NONE YET. This is a mechanism plus a matching anecdote, which is exactly the shape of a story that turns out to be wrong | ⛔ WEAKENED BY L64: `lf em 410x read`, which also has no BLE suspend, is 76/80 (95%) — an eightfold smaller effect than the 4-in-10 this rests on. And it still does not explain the blue dual's 0/6, which is deterministic per tag | L60, L64 |
