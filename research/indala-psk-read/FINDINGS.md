...
| Does a second **physical** tag read? | untested. C23 closes the *structural* half synthetically — parity, constant runs, a preamble inside the payload — but says nothing about a different tag's coupling, tuning or drive level |
| Does `lf indala read` false-positive on a **non-Indala tag**? | ⛔ untested, and the empty field does not test it. An HID/EM/ioProx tag puts a strong ASK or FSK signal on the antenna; whether the Indala decoder hallucinates a credential out of it is a much sharper null than silence |
