...
| F12 | **FSK2a emulation emits a CONSTANT TONE** — HID Prox, ioProx and AWID advertise an emulate path that carries no data at all. ⭐⭐ **CONVICTED AGAINST A REAL TAG (C413)**: a pm3-written HID Prox carrying **the same credential** reads **44.4%** RF/10 on the same chain, same pad, where our emulation reads **7.0%** — and the real tag shows two clean peaks of near-equal height (76 us and 60 us) where ours shows one. The instrument is calibrated and exonerated, so this is a firmware defect in the EMITTER, not an artefact. ⭐ The shape is a **cliff** (C411): pure frames emit either tone correctly, every mixed frame collapses. The cause is still open, now bounded to a sequence mixing 4-entry and 5-entry tones ⭐⭐⭐ **C420 RESOLVES C415'S FORK: THE PLAYBACK PATH IS EXONERATED AND THE ANALOG SIDE IS CONVICTED.** Long-tone survival tracks frame COMPOSITION — 73% / 54% / 58% / 7% / 0% as the long tone goes from all of the frame to a twelfth of it — and a logic defect cannot do that, since the sequence buffer is structurally identical for every composition and `SEQ[0].CNT` is right for all of them (C414). The minority tone dies; the majority tone lives; the cliff sits where the majority flips. ⛔ C411's *any mixing collapses* is corrected, and its rate-independence is explained: all four rate points were 50/50 frames. ⭐ **THE FIX IS NOW JUSTIFIED AND IS C414'S**: `fsk2a_mod.c` fixes the mark at 4 carrier cycles, making RF/8 50% duty and RF/10 40%, and the weaker-duty tone is the one that dies. Make the mark HALF THE TONE PERIOD at both tones — `counter_top` 8 instead of 16, worst case 4800 entries, so `LF_FSK2A_MAX_ENTRIES` must rise with it and F13's guards must be re-pinned against the new bound. ⚠ Pin it in `ctest/roundtrip.c` BEFORE it goes near hardware. ⛔ **C421 WITHDRAWS THE FIX C420 PROPOSED.** The 50%-duty shape has already been on the air and was **0 of 6** (C226), and the mark shape was re-tested across all three FSK protocols with Gallagher 6/6 bracketing and was 0 of 6 again (C380). C420's composition measurement stands; its mechanism does not. ⇒ **The live hypothesis is C380's, never tested: the SWITCHING RATE.** FSK2a switches the modulator at 15.6 kHz where every working emitter on this device runs 3.9 kHz or slower (Gallagher 32, Securakey 40, Noralsy 32, EM410X 64 carrier cycles). Next test scales the tone periods to RF/32 + RF/40 — same 4:5 ratio, same duty, rate alone changed. ⭐⭐⭐⭐ **CAUSE ESTABLISHED (C422): THE SWITCHING RATE.** The same frame reads **3.3% with no long-tone peak at RF/8+RF/10** and **36.4% with two clean peaks at RF/32+RF/40** — ratio, duty, pulse counts and buffer all held constant, only the time scale moved. The receiver is exonerated (C413 read a real tag at 44.4% on this pad), so our EMITTER'S ANALOG DRIVE PATH cannot switch at 15.6 kHz. ⛔ **This does NOT fix AWID** — real AWID *is* RF/8 and RF/10. ⇒ The fix moves to the DRIVE PATH, and WHICH element limits it (PWM output stage, coil drive, tank Q, matching) is not yet separated. ⭐ Not a hard bound: the Flipper emits AWID at these rates well enough for our reader to decode byte-exact (C226). ⭐⭐ **THE KNEE IS LOCATED (C423)**: RF/8+RF/10 **3.3% with no long-tone peak**, RF/12+RF/15 **20.2%**, RF/16+RF/20 **35.4%**, RF/32+RF/40 **36.4%** — monotonic, saturating by RF/16, half-recovery near RF/12, and the failure changes KIND across it (absent at RF/10, attenuated at RF/12). ⇒ AWID's required rate sits about **2x beyond where recovery begins**, and the gradual roll-off says first-order or Q-limited, not a digital cutoff. ⚠ A count fraction, not amplitude — no dB corner is quotable without `rdrcap.py`. ⭐⭐⭐ **THE ELEMENT IS NAMED (C424): THE LF ANTENNA TANK.** `LF_MOD` (P1.13) gates **Q3** switching **R11 220R** onto `LF_OA`, behind series diode **VD1** and tank caps **C34 2.2nF + C10 5.6nF** (L = 208 uH at 125 kHz). The switch is retired — R11 x C28 = **2.2 us** against a 32 us mark — and a tank must band-limit at fc/Q. Required Q is ≤8 for RF/16, ≤6 for RF/12, ≤4 for RF/8, so **a loaded Q of 6-8 reproduces C423's curve exactly**. ⚠ Mechanism established; the Q VALUE is inferred from the knee, so an independent ringdown or -3dB measurement is the falsifiable test. ⛔ **If it confirms, F12 is a HARDWARE limit, not a firmware defect** — damping the tank is a board change. ⭐ The one firmware option left is PRE-EMPHASIS: the load switch is binary but its timing is ours. ⛔⚠ **C426 DOWNGRADES C424/C425 TO CONSISTENT-BUT-UNCONFIRMED.** Their central prediction is **untestable on this bench**: `rdrcap.py --settle 0` captures the field-start transient, but it decays with tau = **30.5 us** on #1 and **29.3 us** on #2 — the same within 4% under radically different antenna coupling, so it is the **filter chain**, not the tank. Every ADC path sits behind that chain, whose pole (~30 us) exceeds the tank's whole predicted tau (13-18 us). A scope on a test point is required. ⇒ F12's cause is **explained and not confirmed**. ⭐ The one avenue that needs no Q value is **PRE-EMPHASIS**, measurable by the same tone-composition method that produced C420-C423. ⛔⛔⭐ **DISPOSITION (C427): CAUSE EXPLAINED, NOT PROVEN; FIRMWARE SURFACE EXHAUSTED; NO FURTHER FIRMWARE WORK JUSTIFIED.** The full enumeration is now closed — encoding (C415), duty (C226/C380/C421), buffer (F13), **pre-emphasis** (needs headroom above steady state; the LF section has ONE binary modulation leg, R11 220R + Q3, so there is no *harder*) and **switch damping** (the antenna switch is SPDT with no high-Z and both throws are supply rails, so parking LOW or HIGH damps identically). ⇒ If C424/C425's tank explanation holds, no firmware change fixes this. ⚠ C426 says that explanation is unconfirmable on this bench, so F12 is a **documented constraint**, not a live defect. ⭐ **What reopens it**: a scope on `LF_OA_OUT` or `LF_ANT_DRV` returning a loaded Q far from 6-8. | `hidprox.c`, `ioprox.c`, `awid.c`, `lf_tag_em.c` | yes — ⛔ **NOT FIXED, characterised only** |
| F13 | **A refused FSK2a modulator was handed straight to EasyDMA** — `lf_fsk2a_build()` returns NULL for any frame it cannot render, `lf_tag_data_loadcb_inner()` assigned that to `m_pwm_seq` unchecked, and **both** `nrfx_pwm_simple_playback()` sites dereferenced it: a NULL read in a release build, where `NRFX_ASSERT` is compiled out. ⚠ Unreachable today by **exactly zero margin** — AWID and HID Prox are 96 bits and an all-ones frame costs 96x25 = **2400** entries against a **2400**-entry buffer. ⛔ **Pyramid is 128 bits and is the next FSK2a emitter in the queue** (U11), so it trips both the frame-bit guard and the buffer guard on its first run | `lf_tag_em.c`, `ctest/roundtrip.c` | yes — both playback sites guard and emit nothing instead of faulting; `trial_fsk_limits()` pins the zero margin and the 128-bit refusal, break-tested by raising both limits |
