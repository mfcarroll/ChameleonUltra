...
| L57 | — | this commit | ⛔⛔⛔ **THE TAG WAS ON THE WRONG SIDE OF THE DEVICE, FOR THE ENTIRE PROJECT. The reading side is the FRONT. It is worth 21x.** Same coil: 1.0–1.1x the empty floor on the back, **20–34x** on the front. The coil that read 0/12 reads first try. Single-capture decode 32% -> **71%**. ⭐ **Sample phase 0 — the stock trigger, the headline "stock firmware would fail even with a correct decoder" — decodes 3/3 on the front**, as do phases 96–120; the only dead band is 56–88 and it carries the HIGHEST skirt, so it is a polarity null rather than weak signal. ⇒ ~26 dB of the "31.2 dB below the Proxmark" was placement. Every measurement in this project was taken at ~1/20 of the available signal; the ledger now marks those ⚠B and `NEXT.md` §1 is the re-measurement. ⚠ The Flipper reads from its back, which is where the assumption came from. |
