...
| L53 | — | this commit | ⛔⛔ **A second Indala tag is inaudible to the Chameleon.** A spare T5577 written with `a0000000e6bd0e92` by a Proxmark — "Data written and verified", then read back as Fmt 26 FC 52 Card 63612 — decoded **0 of 12** on the Chameleon, and its fc/2 skirt sits at 0.89–1.11x the empty-field floor across twelve sample phases where the bench tag sits at 1.25–1.73x. The subcarrier is not there at all. ⚠ Cause unknown (position / tag / reader) and the control has not been run. ⇒ The working margin is only 1.25–1.73x over the floor, so anything worth more than ~1.3x makes a tag unreadable. |
| L54 | — | this commit | ⚠ **The Chameleon's T5577 writer is not known to work.** Three raw `lf_t55xx_write_block` calls landed exactly one block; routing through the proven `write_t55xx` (field held on, every block written twice) changed the spectrum not at all. Not diagnosable from this device — it has no T5577 read, so every attempt costs a Proxmark round trip. `NEXT.md` §7. |
