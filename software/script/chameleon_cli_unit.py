...
# ⭐ Re-measured after C302 made `unpack()` prefer a format that can VALIDATE. Two formats left
# this list because the fix repaired them: KASTLE now reads back as itself always — it was
# C284's headline failure, a Kastle tag reporting as Check Point card 8389632 — and HGEN37
# moved to the "sometimes" set below. ⛔ The list is generated by `ctest/ambig.c`, which fails
# `make check` if these two collections stop matching the firmware's actual behaviour.
WRITE_READS_BACK_AS = {
    ...
    HIDFormat.KANTECH: HIDFormat.HCP32,    HIDFormat.WIE32: HIDFormat.HCP32,
    HIDFormat.N10002: HIDFormat.H10306,    HIDFormat.SMP34: HIDFormat.H10306,
    HIDFormat.BQT34: HIDFormat.H10306,     HIDFormat.H10304: HIDFormat.H10302,
...
WRITE_SOMETIMES_MISNAMED = frozenset({
    HIDFormat.OPTUS34, HIDFormat.SIE36, HIDFormat.P10004, HIDFormat.HGEN37,
})
...
# ⭐ GALLAGHER'S DESCRAMBLE, HOST SIDE. A 256-byte substitution over the eight payload bytes,
# then four bit-field extractions. Transcribed from Momentum's `protocol_gallagher_descramble`
# and checked below against a real credential rather than against itself.
GALLAGHER_LUT = [
    
]


def _gallagher_fields(frame: bytes):
    """(region, facility, card, issue, crc_ok) out of a 96-bit Gallagher frame."""
    ...
    # ⛔ STRIDE 9, not "remove every 9th bit": byte i is the 8 bits at 16 + 9i, so the
    # interleaved parity bit is stepped over. The two descriptions agree on the bytes and
    # disagree on the code.
    raw_bytes = [gb(16 + 9 * i, 8) for i in range(8)]
    crc = gb(16 + 9 * 8, 8)
    c = 0x2C                                  # ⛔ poly 0x07, init 0x2C — verified, not guessed
    for b in raw_bytes:
        c ^= b
        for _ in range(8):
            c = ((c << 1) ^ 0x07) & 0xFF if c & 0x80 else (c << 1) & 0xFF
    p = [GALLAGHER_LUT[b] for b in raw_bytes]
    region = (p[3] & 0x1E) >> 1
    issue = p[7] & 0x0F
    facility = ((p[5] & 0x0F) << 12) | (p[1] << 4) | ((p[7] >> 4) & 0x0F)
    card = (p[0] << 16) | ((p[4] & 0x1F) << 11) | (p[2] << 3) | ((p[3] & 0xE0) >> 5)
    return region, facility, card, issue, (c == crc)


# ⛔ ASSERTED AGAINST A REAL CLONE, NOT AGAINST ITSELF. These are the bytes a Proxmark
# `lf gallagher clone --rc 1 --fc 4321 --cn 6789 --il 2` wrote to this bench's T5577, and the
# four fields its own reader printed back. A descramble checked only against its inverse
# passes with both directions wrong — the exact shape of bug that cost this project a
# fortnight on a PSK decoder whose test encoded with the same mistake it decoded with.
assert _gallagher_fields(bytes.fromhex("7feaa31e76d86c6d868cc249")) == (1, 4321, 6789, 2, True), \
    "Gallagher descramble disagrees with the reference clone"


@lf_gallagher.command("read")
class LFGallagherRead(ReaderRequiredUnit):
    def args_parser(self) -> ArgumentParserNoExit:
        parser = ArgumentParserNoExit()
        parser.description = ("Scan a Gallagher credential (ASK/Manchester, RF/32). ⚠ Slower "
                              "than the PSK reads — the capture is 114ms against 33ms, "
                              "because the length threshold was measured at 10240 samples.")
        return parser

    def on_exec(self, args: argparse.Namespace):
        raw, phase, offset, tries = self.cmd.gallagher_scan()
        region, facility, card, issue, crc_ok = _gallagher_fields(raw)
        print("Gallagher ASK/Manchester")
        print(f"   Raw: {color_string((CY, raw.hex()))}")
        print(f"   Region: {color_string((CG, region))}  Facility: {color_string((CY, facility))}  "
              f"Card: {color_string((CY, card))}  Issue: {color_string((CG, issue))}")
        if not crc_ok:
            # ⚠ Should be unreachable — the firmware gates on this CRC — so if it ever prints,
            # the host and firmware disagree about the frame, which is worth knowing loudly.
            print(f"   {color_string((CR, 'CRC MISMATCH'))} — the firmware accepted a frame "
                  f"the host rejects. That is a disagreement between them, not a bad tag.")
        print(f"   Read at sample phase {phase} ticks, bit offset {offset}, "
              f"{tries} capture{'' if tries == 1 else 's'} taken")


@lf_gallagher.command("write")
class LFGallagherWrite(ReaderRequiredUnit):
    def args_parser(self) -> ArgumentParserNoExit:
        parser = ArgumentParserNoExit()
        parser.description = ("Write a Gallagher credential to a T5577. Configures ASK, "
                              "RF/32, 3 data blocks (00088060).")
    ...
    def on_exec(self, args: argparse.Namespace):
        ...
        if not crc_ok:
            # ⚠ A warning, not a refusal: a raw write is the escape hatch for testing exactly
            # this. But no Gallagher reader will accept it, so say so.
            print(f"   {color_string((CY, 'WARNING'))}: this frame's CRC does not check out, "
                  f"so no Gallagher reader will accept it.")

        before = self._try_read()
        if before is None:
            print(f"   {color_string((CY, 'No Gallagher frame before the write.'))} Expected "
                  f"for a blank or non-Gallagher tag, but it also means there is no proof "
                  f"this tag is coupled — so a failure after the write will not be "
                  f"distinguishable from a tag the reader cannot hear.")
        else:
            print(f"   before: {color_string((CY, before))}")

        self.cmd.gallagher_write_to_t55xx(frame)

        after = self._try_read()
        if after == hexs:
            print(f"{color_string((CG, 'VERIFIED'))} region {region} facility {facility} "
                  f"card {card} issue {issue} — read back off the tag.")
        elif after is None and before is None:
            print(f"{color_string((CY, 'CANNOT TELL'))} — nothing readable before or after. "
                  f"Reposition the tag and try {color_string((CG, 'lf gallagher read'))}.")
        elif after is None:
            print(f"{color_string((CR, 'WRITE FAILED'))} — the tag read {before} before and "
                  f"nothing after.")
        elif after == before:
    ...
    def _try_read(self):
        """The Gallagher frame on the tag, or None. ⚠ None is NOT proof of absence."""
        try:
            return self.cmd.gallagher_scan()[0].hex()
        except Exception:
            return None
