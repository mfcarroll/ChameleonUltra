...
/* ⛔ Instrumentation for C148 — see lf_125khz_radio.c. Reads the LF radio's drive value and
 * PWM0's live registers, which is the one measurement that says whether the value is wrong or
 * the peripheral is ignoring a correct value. */
static data_frame_tx_t *cmd_processor_lf_radio_debug(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    uint8_t buf[LF_RADIO_DEBUG_SIZE] = { 0x00 };
...
static data_frame_tx_t *cmd_processor_indala224_scan(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    status = scan_indala224(card_data);
    if (status != STATUS_LF_TAG_OK) {
        return data_frame_make(cmd, status, 0, NULL);
    }
    return data_frame_make(cmd, STATUS_LF_TAG_OK, sizeof(card_data), card_data);
...
static data_frame_tx_t *cmd_processor_fdxa_scan(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    uint8_t card_data[FDXA_READ_DATA_SIZE] = { 0x00 };
    status = scan_fdxa(card_data);
    if (status != STATUS_LF_TAG_OK) {
        return data_frame_make(cmd, status, 0, NULL);
    }
    return data_frame_make(cmd, STATUS_LF_TAG_OK, sizeof(card_data), card_data);
...
static data_frame_tx_t *cmd_processor_lf_reader_capture(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    uint16_t gap = (length >= 8) ? data[7] : 0;

    if (chunk == 0) {
        nsamp = 0;
        lf_reader_capture_probe(want, drive, phase, repeats, settle, gap, &buf, &nsamp);
    }
    if (buf == NULL || nsamp == 0) {
...
static data_frame_tx_t *cmd_processor_gproxii_write_to_t55xx(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    typedef struct {
        uint8_t frame12[12];
        uint8_t new_key[4];
        uint8_t old_keys[4];
    ...
    uint8_t old_cnt = (length - offsetof(payload_t, old_keys)) / sizeof(payload->old_keys);
    status = write_gproxii_to_t55xx(payload->frame12, payload->new_key, payload->old_keys, old_cnt);
    return data_frame_make(cmd, status, 0, NULL);
}
...
static data_frame_tx_t *cmd_processor_noralsy_scan(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    uint8_t card_data[NORALSY_READ_DATA_SIZE] = { 0x00 };
    status = scan_noralsy(card_data);
    if (status != STATUS_LF_TAG_OK) {
        return data_frame_make(cmd, status, 0, NULL);
    }
    return data_frame_make(cmd, STATUS_LF_TAG_OK, sizeof(card_data), card_data);
...
static data_frame_tx_t *cmd_processor_noralsy_write_to_t55xx(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    typedef struct {
        uint8_t frame12[12];
        uint8_t new_key[4];
        uint8_t old_keys[4];
    ...
    uint8_t old_cnt = (length - offsetof(payload_t, old_keys)) / sizeof(payload->old_keys);
    status = write_noralsy_to_t55xx(payload->frame12, payload->new_key, payload->old_keys, old_cnt);
    return data_frame_make(cmd, status, 0, NULL);
}

static data_frame_tx_t *cmd_processor_securakey_scan(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    uint8_t card_data[SECURAKEY_READ_DATA_SIZE] = { 0x00 };
    status = scan_securakey(card_data);
    if (status != STATUS_LF_TAG_OK) {
        return data_frame_make(cmd, status, 0, NULL);
    }
    return data_frame_make(cmd, STATUS_LF_TAG_OK, sizeof(card_data), card_data);
...
static data_frame_tx_t *cmd_processor_securakey_write_to_t55xx(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    typedef struct {
        uint8_t frame12[12];
        uint8_t new_key[4];
        uint8_t old_keys[4];
    ...
    uint8_t old_cnt = (length - offsetof(payload_t, old_keys)) / sizeof(payload->old_keys);
    status = write_securakey_to_t55xx(payload->frame12, payload->new_key, payload->old_keys, old_cnt);
    return data_frame_make(cmd, status, 0, NULL);
}

static data_frame_tx_t *cmd_processor_gallagher_scan(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    uint8_t card_data[GALLAGHER_READ_DATA_SIZE] = { 0x00 };
    status = scan_gallagher(card_data);
    if (status != STATUS_LF_TAG_OK) {
        return data_frame_make(cmd, status, 0, NULL);
    }
    return data_frame_make(cmd, STATUS_LF_TAG_OK, sizeof(card_data), card_data);
...
static data_frame_tx_t *cmd_processor_gallagher_write_to_t55xx(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    typedef struct {
        uint8_t frame12[12];
        uint8_t new_key[4];
        uint8_t old_keys[4];
    ...
    uint8_t old_cnt = (length - offsetof(payload_t, old_keys)) / sizeof(payload->old_keys);
    status = write_gallagher_to_t55xx(payload->frame12, payload->new_key, payload->old_keys, old_cnt);
    return data_frame_make(cmd, status, 0, NULL);
}

static data_frame_tx_t *cmd_processor_nexwatch_scan(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    uint8_t card_data[NEXWATCH_READ_DATA_SIZE] = { 0x00 };
    status = scan_nexwatch(card_data);
    if (status != STATUS_LF_TAG_OK) {
        return data_frame_make(cmd, status, 0, NULL);
    }
    return data_frame_make(cmd, STATUS_LF_TAG_OK, sizeof(card_data), card_data);
...
static data_frame_tx_t *cmd_processor_nexwatch_write_to_t55xx(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    typedef struct {
        uint8_t frame12[12];  /* the 96-bit AIR frame, big-endian, preamble included */
        uint8_t new_key[4];
        uint8_t old_keys[4];  /* one or more */
    } PACKED payload_t;
    ...
    uint8_t old_cnt = (length - offsetof(payload_t, old_keys)) / sizeof(payload->old_keys);
    status = write_nexwatch_to_t55xx(payload->frame12, payload->new_key, payload->old_keys, old_cnt);
    return data_frame_make(cmd, status, 0, NULL);
}
...
/* ⚠ A SEPARATE COMMAND RATHER THAN A FLAG ON 3034. The payload is a fixed-size raw frame
 * followed by a variable number of old keys, and the only thing distinguishing "28-byte
 * frame plus one key" from "8-byte frame plus six keys" is which command asked. A length
 * byte would make the two ambiguous on a truncated frame; a distinct id cannot be. */
static data_frame_tx_t *cmd_processor_indala224_write_to_t55xx(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    typedef struct {
        uint8_t raw28[28];    /* the 224-bit frame, big-endian */
        uint8_t new_key[4];
        uint8_t old_keys[4];  /* one or more */
    } PACKED payload_t;
    ...
    uint8_t old_cnt = (length - offsetof(payload_t, old_keys)) / sizeof(payload->old_keys);
    status = write_indala224_to_t55xx(payload->raw28, payload->new_key, payload->old_keys, old_cnt);
    return data_frame_make(cmd, status, 0, NULL);
}
...
static data_frame_tx_t *cmd_processor_noralsy_set_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    if (length != LF_NORALSY_TAG_ID_SIZE) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_NORALSY);
    memcpy(buffer->buffer, data, LF_NORALSY_TAG_ID_SIZE);
    tag_emulation_load_by_buffer(TAG_TYPE_NORALSY, false);
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}

static data_frame_tx_t *cmd_processor_noralsy_get_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    tag_slot_specific_type_t tag_types;
    tag_emulation_get_specific_types_by_slot(tag_emulation_get_slot(), &tag_types);
    if (tag_types.tag_lf != TAG_TYPE_NORALSY) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, data);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_NORALSY);
    return data_frame_make(cmd, STATUS_SUCCESS, LF_NORALSY_TAG_ID_SIZE, buffer->buffer);
}

static data_frame_tx_t *cmd_processor_securakey_set_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    if (length != LF_SECURAKEY_TAG_ID_SIZE) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_SECURAKEY);
    memcpy(buffer->buffer, data, LF_SECURAKEY_TAG_ID_SIZE);
    tag_emulation_load_by_buffer(TAG_TYPE_SECURAKEY, false);
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}

static data_frame_tx_t *cmd_processor_securakey_get_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    tag_slot_specific_type_t tag_types;
    tag_emulation_get_specific_types_by_slot(tag_emulation_get_slot(), &tag_types);
    if (tag_types.tag_lf != TAG_TYPE_SECURAKEY) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, data);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_SECURAKEY);
    return data_frame_make(cmd, STATUS_SUCCESS, LF_SECURAKEY_TAG_ID_SIZE, buffer->buffer);
}

static data_frame_tx_t *cmd_processor_gallagher_set_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    if (length != LF_GALLAGHER_TAG_ID_SIZE) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_GALLAGHER);
    memcpy(buffer->buffer, data, LF_GALLAGHER_TAG_ID_SIZE);
    tag_emulation_load_by_buffer(TAG_TYPE_GALLAGHER, false);
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}

static data_frame_tx_t *cmd_processor_gallagher_get_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    tag_slot_specific_type_t tag_types;
    tag_emulation_get_specific_types_by_slot(tag_emulation_get_slot(), &tag_types);
    if (tag_types.tag_lf != TAG_TYPE_GALLAGHER) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, data);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_GALLAGHER);
    return data_frame_make(cmd, STATUS_SUCCESS, LF_GALLAGHER_TAG_ID_SIZE, buffer->buffer);
}

static data_frame_tx_t *cmd_processor_gproxii_set_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    if (length != LF_GPROXII_TAG_ID_SIZE) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_GPROXII);
    memcpy(buffer->buffer, data, LF_GPROXII_TAG_ID_SIZE);
    tag_emulation_load_by_buffer(TAG_TYPE_GPROXII, false);
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}

static data_frame_tx_t *cmd_processor_gproxii_get_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    tag_slot_specific_type_t tag_types;
    tag_emulation_get_specific_types_by_slot(tag_emulation_get_slot(), &tag_types);
    if (tag_types.tag_lf != TAG_TYPE_GPROXII) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, data);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_GPROXII);
...
static data_frame_tx_t *cmd_processor_awid_set_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_AWID);
    memcpy(buffer->buffer, data, LF_AWID_TAG_ID_SIZE);
    tag_emulation_load_by_buffer(TAG_TYPE_AWID, false);
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}

static data_frame_tx_t *cmd_processor_awid_get_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    tag_slot_specific_type_t tag_types;
    tag_emulation_get_specific_types_by_slot(tag_emulation_get_slot(), &tag_types);
    if (tag_types.tag_lf != TAG_TYPE_AWID) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, data);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_AWID);
    return data_frame_make(cmd, STATUS_SUCCESS, LF_AWID_TAG_ID_SIZE, buffer->buffer);
}
...
static data_frame_tx_t *cmd_processor_indala224_set_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    if (length != LF_INDALA224_TAG_ID_SIZE) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_INDALA224);
    memcpy(buffer->buffer, data, LF_INDALA224_TAG_ID_SIZE);
    tag_emulation_load_by_buffer(TAG_TYPE_INDALA224, false);
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}

static data_frame_tx_t *cmd_processor_indala224_get_emu_id(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    tag_slot_specific_type_t tag_types;
    tag_emulation_get_specific_types_by_slot(tag_emulation_get_slot(), &tag_types);
    if (tag_types.tag_lf != TAG_TYPE_INDALA224) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, data);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(TAG_TYPE_INDALA224);
    return data_frame_make(cmd, STATUS_SUCCESS, LF_INDALA224_TAG_ID_SIZE, buffer->buffer);
}
...
static cmd_data_map_t m_data_cmd_map[] = {
    ...
    {    DATA_CMD_GALLAGHER_SCAN,               before_reader_run,           cmd_processor_gallagher_scan,                NULL                   },
    {    DATA_CMD_GALLAGHER_WRITE_TO_T55XX,     before_reader_run,           cmd_processor_gallagher_write_to_t55xx,      NULL                   },
    {    DATA_CMD_SECURAKEY_SCAN,               before_reader_run,           cmd_processor_securakey_scan,                NULL                   },
    {    DATA_CMD_SECURAKEY_WRITE_TO_T55XX,     before_reader_run,           cmd_processor_securakey_write_to_t55xx,      NULL                   },
    {    DATA_CMD_NORALSY_SCAN,                 before_reader_run,           cmd_processor_noralsy_scan,                  NULL                   },
    ...
    {    DATA_CMD_GALLAGHER_SET_EMU_ID,           NULL,                      cmd_processor_gallagher_set_emu_id,          NULL                   },
    {    DATA_CMD_AWID_SET_EMU_ID,                NULL,                      cmd_processor_awid_set_emu_id,               NULL                   },
    ...
    {    DATA_CMD_NORALSY_SET_EMU_ID,             NULL,                      cmd_processor_noralsy_set_emu_id,            NULL                   },
    {    DATA_CMD_NORALSY_GET_EMU_ID,             NULL,                      cmd_processor_noralsy_get_emu_id,            NULL                   },
